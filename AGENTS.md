...
Causa is a working prototype (see `README.md` and `docs/architecture.md`). The
full pipeline is built: RCA contract, payments demo app, observability stack,
triage (mock + MCP), investigators (mock + Cursor SDK), FastAPI API, and
Streamlit console. Run via `task demo` or `task run:local`. Do **not** run a bare
`docker compose up` — bring up the substrate subset listed in the README.
...
- Validate fixture / regenerate schema: `task check`
...
Bring up the substrate with `task substrate:up` or as documented in the README —
list services explicitly; do **not** run a bare `docker compose up`.
